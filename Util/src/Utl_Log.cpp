...
{
    int rv = UTL_LOG_OK, fd = -1;
    char shmName[32];
    void *addr = NULL;

    UTLSlimRWLock_LockWrite(&utl_logLock);

    if (enable == utl_enableInMemoryLogging) {
        // No change, exit
        goto exit;
    }

    // Add the ID to the name in case there are multiple CCD instances
    snprintf(shmName, ARRAY_SIZE_IN_BYTES(shmName), "%s_%d", UTL_LOG_SHM_OBJECT_NAME, id);

    if (enable) {
        fd = UTLShm_Open(shmName, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
        if (fd < 0) {
            rv = UTL_LOG_ERR_SHM;
            goto exit;
        }

        rv = UTL_Fallocate(fd, 0, UTL_LOG_SHM_OBJECT_SIZE);
        if (rv < 0) {
            rv = UTL_LOG_ERR_SHM;
            goto exit;
        }

        rv = UTL_Mmap(NULL, UTL_LOG_SHM_OBJECT_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0, &addr);
        if (rv < 0) {
            rv = UTL_LOG_ERR_SHM;
            goto exit;
        }

        utl_shmHeader = (UTL_LOGShmHeader *) addr;
        utl_shmFd = fd;

        // Initialize SHM if it's not already valid
        if (memcmp(utl_shmHeader->magic, shmMagic, ARRAY_SIZE_IN_BYTES(utl_shmHeader->magic)) != 0) {
            utl_shmHeader->version = 1;
            utl_shmHeader->readOff = sizeof(UTL_LOGShmHeader);
            utl_shmHeader->writeOff = sizeof(UTL_LOGShmHeader);
            utl_shmHeader->wrapped = false;
            utl_shmHeader->logSize = 0;
            memcpy(utl_shmHeader->magic, shmMagic, ARRAY_SIZE_IN_BYTES(utl_shmHeader->magic));
        }
    } else {
        // Flush out existing logs
        UTL_logFlushInMemoryLogsHelper();

        memset(utl_shmHeader->magic, 0, ARRAY_SIZE_IN_BYTES(utl_shmHeader->magic));
        UTL_Munmap((void *) utl_shmHeader, UTL_LOG_SHM_OBJECT_SIZE);
        utl_shmHeader = NULL;

        UTL_Close(utl_shmFd);
        utl_shmFd = -1;

        UTLShm_Unlink(shmName);
    }

    utl_enableInMemoryLogging = enable;

exit:
    if (rv != UTL_LOG_OK && enable) {
        if (addr != NULL) {
            UTL_Munmap(addr, UTL_LOG_SHM_OBJECT_SIZE);
        }
